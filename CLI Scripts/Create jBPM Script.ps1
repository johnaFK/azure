...
$RGid =$(az group list --query "[?name=='jbpm-test-rg'].[id]" --output tsv)
...
az vm extension set `
  ...
  --publisher Microsoft.Azure.Extensions `
  --resource-group $RGName `
  --settings '{\"fileUris\":[\"https://raw.githubusercontent.com/johnaFK/azure/CLI-Scripts/CLI%20Scripts/install-jbpm-on-linux.sh\"], \"commandToExecute\": \"./install-jbpm-on-linux.sh\"}' `
  --vm-name $VMName 
